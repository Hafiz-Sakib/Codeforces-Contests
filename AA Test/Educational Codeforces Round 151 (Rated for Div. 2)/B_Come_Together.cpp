...
void Boom()
{
    ...
    int minX = min(x2, x3), maxX = max(x2, x3);
    int minY = min(y2, y3), maxY = max(y2, y3);

    cnt += (y1 < minY) ? (minY - y1) : 0;
    cnt += (y1 > maxY) ? (y1 - maxY) : 0;
    cnt += (x1 < minX) ? (minX - x1) : 0;
    cnt += (x1 > maxX) ? (x1 - maxX) : 0;
